...
public class PreferencesService {
    ...
    public UserPreferences loadPreferences() {
        try (Connection connection = databaseManager.getConnection();
             PreparedStatement statement = connection.prepareStatement(
                 "SELECT theme, auto_login, notifications_enabled, dark_overlay, zoom_level FROM preferences WHERE id = 1")) {
            ResultSet rs = statement.executeQuery();
            if (!rs.next()) {
                return new UserPreferences("LIGHT", true, true, false, 1.0);
            }
    ...
    public void savePreferences(UserPreferences preferences) {
        try (Connection connection = databaseManager.getConnection();
             PreparedStatement statement = connection.prepareStatement("""
                 UPDATE preferences
                 SET theme = ?, auto_login = ?, notifications_enabled = ?, dark_overlay = ?, zoom_level = ?
                 WHERE id = 1
                 """)) {
